...
- The backend running and reachable on the network (Pi:
  `192.168.1.50:8080`)
...
| Field | Example |
|---|---|
| `device.id` | `"kali-macbook-air"` (must be persistent and unique) |
| `device.name` | `"Kali MacBook Air 2015"` |
| `backend.url` | `"http://192.168.1.50:8080/api/v1/metrics"` (Pi LAN IP) |
...
| `targets.gateway_ip` | Router LAN IP (e.g. `192.168.1.1`) |
| `targets.ap_ip` | TP-Link TL-WR850N IP |
...
1. Run the service as your normal user (`firat`).
...
- **Backend connection refused:** wrong Pi IP/port; check firewall.
